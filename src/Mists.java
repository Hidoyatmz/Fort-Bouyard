...
class Mists extends Program {
    ...
    boolean allUnactivePylonnes(Game game){
        return getUnactivePylonnes(game) == NB_PYLONNES;
    }

    int getUnactivePylonnes(Game game){
    ...
    void getPylonnesAroundPlayers(Game g){
    ...
    boolean checkPlayersPylonnes(Game game){
        Player[] players = game.players;
        int cpt = 0;
        for(int i = 0; i < length(players); i++){
            if(pylonnesPlayers[i] != null){
    ...
    boolean hasPylonneAround(Player player){
    ...
    String[] getCellsAroundPlayer(Player player){
    ...
    int[] getPylonnePosAroundPlayer(Player player){
        int[] res = new int[2];
        res[0] = 0;
        res[1] = 0;
        int posX = player.posX;
    ...
    void movePlayer(Game game, int id, char direction){
        Player playerToMove = getPlayerById(game, id);
    ...
    Player getPlayerById(Game game, int id){
        Player[] playersArray = game.players;
        Player player = playersArray[0];
        boolean res = false;
        int i = 0;
        do {
    ...
    Game newGame(int[] dimensions, int nbPlayers, int nbPylonnes) {
        int cptPylonnes = 0;
        int idxHiddens = 0;
        Game game = new Game();
    ...
    Player newPlayer(int id, int posX, int posY){
